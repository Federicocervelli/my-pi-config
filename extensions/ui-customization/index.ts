    ...
    ctx.ui.setFooter((tui, theme, footerData: ReadonlyFooterDataProvider) => {
      ...
      return {
        ...
        render(width: number) {
          ...
          const usage = [
            theme.fg(contextColor, contextDisplay),
            modelInfo.provider === "openai-codex" ? codexDisplay : "",
            modelInfo.provider === "openrouter" && modelInfo.cost > 0
              ? theme.fg("accent", formatCost(modelInfo.cost))
              : "",
          ].filter(Boolean).join(" · ");
