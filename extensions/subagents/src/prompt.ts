...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background pi subagent: a fully autonomous, headless agent with its own context window and this environment's normal tools and config. Fire-and-forget: this returns immediately with an id. The subagent's final output is queued back to you as a message when it settles, or collect it explicitly with subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Only use trusted working directories. Max 4 subagents.";
...
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  ...
  "Use the pi harness for every subagent task.",
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  model:
    'Model hint for pi ("provider/model-id" or model id). Omit for the harness default (pi inherits the current model).',
  reasoningEffort:
    "Reasoning effort / thinking level for pi. Omit for the harness default (pi inherits the current level).",
};
