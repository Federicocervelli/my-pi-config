/**
 * Subagents — spawn background subagents through the pi backend.
 *
 ...
 * settle. `/sa` opens a picker + full interactive takeover view.
 *
...
export default function (pi: ExtensionAPI, initialCtx?: ExtensionContext) {
  let runtime: SubagentRuntime | undefined;
  let managerPromise: Promise<SubagentManagerShape> | undefined;
  let sessionContext: ExtensionContext | undefined;
  let ui: ExtensionUIContext | undefined;
  if (initialCtx) {
    sessionContext = initialCtx;
    if (initialCtx.hasUI) ui = initialCtx.ui;
  }
  ...
  const goalBridge: GoalSubagentBridge = {
    spawn: async (task) =>
      runTool(
        getRuntime(),
        (await getManager()).spawn("pi", makeSpawnTask(task)),
        { interruptMessage: "Goal subagent spawn was aborted." },
      ),
    waitFor: async (ids) =>
      runTool(
        getRuntime(),
        (await getManager()).waitFor(ids),
        { interruptMessage: "Goal subagent wait was aborted." },
      ),
    get: async (id) => (await getManager()).view.get(id),
    cancel: async (ids) =>
      runTool(
        getRuntime(),
        (await getManager()).cancel(ids),
        { interruptMessage: "Goal subagent cancellation was aborted." },
      ),
  };
  const stopGoalService = pi.events.on(GOAL_SUBAGENT_SERVICE, (event) => {
    if (
      event &&
      typeof event === "object" &&
      "provide" in event &&
      typeof event.provide === "function"
    ) {
      (event as GoalSubagentServiceRequest).provide(goalBridge);
    }
  });
  ...
  const updateStatus = (manager: SubagentManagerShape) => {
    ...
    ui.setStatus("subagents", formatActivityStatus(ui.theme, { running }));
  };
  ...
  const deliverBtwResult = (snap: SubagentSnapshot) => {
    ...
    ui?.notify(
      snap.status === "error"
        ? `by the way “${snap.title}” failed — reopen it with /sa`
        : `by the way “${snap.title}” answered — reopen it with /sa`,
